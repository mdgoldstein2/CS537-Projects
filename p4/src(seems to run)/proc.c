...
//        cprintf("new process bby\n");
    ...
    if (ptable.proc_queue.tail->pid != 1) {
//        cprintf("enqueue %d\n", ptable.proc_queue.tail->pid);
    }
}
...
dequeue() {
    ...
    // normally would be removed, added for testing purposes
    if (temp->pid != 1) {
    //    cprintf("dequeue %d\n", temp->pid);
    }
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    if (p != 0 && p->state == RUNNABLE &&
      ...
      //      cprintf("run init\n");
        ...
        // if process did exist, reset runticks and currentcomp, enqueue
        if (p != 0) {
            p->runticks = 0;
            p->currentcomp = 0;
            enqueue(p);
    ...
    //            cprintf("sleepy\n");
        ...
        if (p == 0 || p->state != RUNNABLE) {  // queue is empty in this case, so continue
            release(&ptable.lock);
            continue;
        }
  ...
  //          cprintf("run init\n");
    ...
    // normally would be removed, uses old scheduler but with ticks for testing
    /*else {
        if (p != 0) {
            p->runticks = 0;
            p->currentcomp = 0;
