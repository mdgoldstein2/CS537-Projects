...
/**
 * The main method of the shell. Reads in commands from stdin or a batch file
 * and executes them. Also checks the length of the commands and sets up/cleans
 * data structures needed for executing.
 *
...
int main(int argc, char* argv[]) {
    ...
    while (fgets(input_buffer, kBufferSize, input_file) != NULL) {
        ...
        if (strlen(input_buffer) > kMaxLineLength) {
            ...
            // clear out the rest of the line if necessary (if the oversized
            // line did not fit into the slightly extended buffer)
